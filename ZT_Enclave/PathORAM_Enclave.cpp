...
/*
PathORAM::PathORAM(uint32_t s_max_blocks, uint32_t s_data_size, uint32_t
s_stash_size, uint32_t oblivious, uint32_t s_recursion_data_size, int8_t
recursion_levels, uint64_t onchip_posmap_mem_limit){ max_blocks = s_max_blocks;
...
void PathORAM::Initialize(uint32_t instance_id, uint8_t oram_type, uint8_t pZ,
...
void PathORAM::Create(uint32_t instance_id, uint8_t oram_type, uint8_t Z,
                      uint32_t max_blocks, uint32_t data_size,
                      uint32_t stash_size, uint32_t oblivious_flag,
                      uint32_t recursion_data_size, uint8_t recursion_levels) {
  Initialize(instance_id, oram_type, Z, max_blocks, data_size, stash_size,
             oblivious_flag, recursion_data_size, recursion_levels);
}

uint32_t PathORAM::access_oram_level(char opType, uint32_t leaf, uint32_t id,
  ...
  return_value = PathORAM_Access(opType, id, position_in_id, leaf, newleaf,
                                 newleaf_nextleaf, decrypted_path, path_hash,
                                 level, data_in, data_out);
  return return_value;
}

uint32_t PathORAM::access(uint32_t id, int32_t position_in_id, char opType,
  ...
  if (recursion_levels == 1) {
    ...
    PathORAM_Access(opType, id, -1, leaf, newleaf, -1, decrypted_path,
                    path_hash, 0, data_in, data_out);
    return 0;
  } else {
    if (level == 0) {
      ...
      newleaf =
          N_level[1] + (*((uint32_t *)random_value) % (N_level[level + 1]));
      ...
      *prev_sampled_leaf = newleaf;
...
#ifdef ACCESS_DEBUG
      printf("access : Level = %d: \n Requested_id = %d, Corresponding leaf "
             "from posmap = %d, Newleaf assigned = %d,\n\n",
      ...
      id_adj = id / x;
      position_in_id = id % x;
      leaf = access(id_adj, position_in_id, opType, level - 1, data_in,
      ...
      leaf = access(id_adj, pos_in_id, opType, level - 1, data_in, data_out,
                    prev_sampled_leaf);
...
void PathORAM::Access(uint32_t id, char opType, unsigned char *data_in,
                      unsigned char *data_out) {
...
uint32_t PathORAM::PathORAM_Access(char opType, uint32_t id,
                                   uint32_t position_in_id, uint32_t leaf,
                                   uint32_t newleaf, uint32_t newleaf_nextlevel,
  ...
  PushBlocksFromPathIntoStash(decrypted_path_ptr, level, tdata_size,
                              tblock_size, id, position_in_id, leaf, &nextLeaf,
                              newleaf, sampledLeaf, newleaf_nextlevel);

  if (oblivious_flag) {
    if (level == recursion_levels - 1) {
      recursive_stash[level].PerformAccessOperation(opType, id, newleaf,
                                                    data_in, data_out);
      // Optional TODO : Add layer of encryption to result, such that only real
      // client (outside server stack) can decrypt.
    } else {
      OAssignNewLabelToBlock(id, position_in_id, level, newleaf,
  ...
  PathORAM_RebuildPath(decrypted_path_ptr, tdata_size, tblock_size, leaf,
                       level);
...
// Encrypt and Upload Path :
  ...
  *(req_struct->block) = false;
  ...
  createNewPathHash(path_ptr, path_hash, new_path_hash, leaf,
                    data_size + ADDITIONAL_METADATA_SIZE, level);
...
void PathORAM::PathORAM_RebuildPath(unsigned char *decrypted_path_ptr,
                                    uint32_t data_size, uint32_t block_size,
                                    uint32_t leaf, uint32_t level) {
  uint32_t prefix;
  uint32_t i, k;
  uint32_t d = D_level[level];
  ...
  for (i = 0; i < d; i++) {
    ...
    if (oblivious_flag) {
      ...
      for (k = 0; k < stash_size; k++) {
        ...
        uint32_t jprefix =
            ShiftBy(getTreeLabel(listptr->serialized_block) + n, i);
        uint32_t sblock_written = false;

        bool flag = (posk < Z) && (prefix == jprefix) && (!sblock_written) &&
                    (!isBlockDummy(listptr->serialized_block, gN));
        for (uint8_t l = 0; l < Z; l++) {
          flag = (l == posk) && (posk < Z) && (prefix == jprefix) &&
                 (!sblock_written) &&
                 (!isBlockDummy(listptr->serialized_block, gN));
          ...
          if (flag) {
            printf("Block %d,%d TO Bucket %d\n",
                   getId(listptr->serialized_block),
                   getTreeLabel(listptr->serialized_block), prefix);
          }
#endif

          omove_serialized_block(decrypted_path_temp_iterator,
                                 listptr->serialized_block, data_size, flag);
      ...
      while (listptr && posk < Z) {
        ...
        if (flag) {
          memcpy(decrypted_path_temp_iterator, listptr->serialized_block,
                 block_size);
    ...
    /*
      ...
      for(uint8_t e =0;e<Z;e++) {
        printf("(%d,%d) ,
    ",getId(decrypted_path_temp_iterator),getTreeLabel(decrypted_path_temp_iterator));
        decrypted_path_temp_iterator+=block_size;
      }
      printf("\n");
    #endif
