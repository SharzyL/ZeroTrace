...
Stash::Stash(uint32_t param_stash_data_size, uint32_t param_STASH_SIZE,
             uint32_t param_gN) {
...
void Stash::setParams(uint32_t param_stash_data_size, uint32_t param_STASH_SIZE,
                      uint32_t param_gN) {
  stash_data_size = param_stash_data_size;
  STASH_SIZE = param_STASH_SIZE;
  gN = param_gN;
...
void Stash::PerformAccessOperation(char opType, uint32_t id, uint32_t newleaf,
...
#ifdef RESULTS_DEBUG
  printf("Before PerformAccess, datasize = %d, Fetched Data :",
         stash_data_size);
...
uint32_t Stash::displayStashContents(uint64_t nlevel, bool recursive_block) {
  ...
  while (iter && cntr <= STASH_SIZE) {
    ...
    if ((!isBlockDummy(iter->serialized_block, gN))) {
      printf("loc = %d, (%d,%d) : ", cntr, getId(iter->serialized_block),
             getTreeLabel(iter->serialized_block));
        ...
        if(start==NULL)	{
                ...
                while(iter) {
                        ...
                        if(!(iter->block->isDummy())&&iter->occupied) {
                                ...
                                ptr+=4;
                                memcpy(stash_serialized,iter->block->data,
datasize_ss); ptr+=datasize_ss;
                        }
                        iter=iter->next;
                        i++;
                }
...
struct nodev2 *Stash::getStart() { return start; }

void Stash::setStart(struct nodev2 *new_start) { start = new_start; }
...
#endif
  while (iter && cntr <= STASH_SIZE) {
    bool flag = (!is_dummy && (isBlockDummy(iter->serialized_block, gN)) &&
                 !block_written);
#ifdef STASH_OVERFLOW_DEBUG
    inserted = inserted || flag;
#endif
    stash_serialized_insert(iter->serialized_block, serialized_block,
                            stash_data_size, flag, &block_written);
    iter = iter->next;
    cntr++;
  }
#ifdef STASH_OVERFLOW_DEBUG
