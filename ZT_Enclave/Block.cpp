...
bool Block::isDummy(uint32_t gN) { return (id == gN); }
...
void Block::serializeToBuffer(unsigned char *serialized_block,
                              uint32_t data_size) {
  unsigned char *ptr = serialized_block;
  memcpy(ptr, (void *)r, NONCE_LENGTH);
  ptr += NONCE_LENGTH;
...
void Block::aes_enc(uint32_t data_size, unsigned char *aes_key) {
  ...
  ret = sgx_aes_ctr_encrypt((const sgx_aes_ctr_128bit_key_t *)aes_key,
                            input_buffer, input_size, ctr, ctr_inc_bits,
                            ciphertext);
...
void Block::aes_dec(uint32_t data_size, unsigned char *aes_key) {
  ...
  ret =
      sgx_aes_ctr_decrypt((const sgx_aes_ctr_128bit_key_t *)aes_key, ciphertext,
                          ciphertext_size, ctr, ctr_inc_bits, input_buffer);
...
unsigned char *Block::getDataPtr() { return data; }
