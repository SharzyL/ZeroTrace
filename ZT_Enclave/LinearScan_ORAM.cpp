...
LinearScan_ORAM::LinearScan_ORAM(uint32_t instance_id, uint32_t key_size_p,
                                 uint32_t value_size_p, uint32_t num_blocks_p,
                                 uint8_t mode_p, uint8_t oblv_mode,
                                 uint8_t dummy_populate = true) {
  ...
  case OUTSIDE_PRM: {
    ...
    ret = createLSORAM_OCALL((void **)&LSORAM_store, instance_id, key_size,
                             value_size, num_blocks, oblivious_mode);
    if (oblv_mode == FULL_OBLV) {
...
uint32_t LinearScan_ORAM::getKeySize() { return key_size; }

uint32_t LinearScan_ORAM::getValueSize() { return value_size; }
...
//  if current size = num_blocks, realloc and update num_blocks (by doubling
//  current size?)
int8_t LinearScan_ORAM::insert(unsigned char *key, uint32_t key_size_p,
                               unsigned char *value, uint32_t value_size_p) {
...
int8_t LinearScan_ORAM::fetch(unsigned char *key, uint32_t key_size_p,
                              unsigned char *value, uint32_t value_size_p) {
  ...
  for (std::vector<tuple *>::iterator it = LSORAM_store->begin();
       it != LSORAM_store->end(); ++it) {
    /*
    printf("Index %d\n", ctr);
    ...
    */
    ...
    // Perform oblivious pass over iterator->key and key, and then
    // iterator->value and value
