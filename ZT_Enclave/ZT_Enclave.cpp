...
bool generateAndSealKeys(unsigned char *bin_x_p, unsigned char *bin_y_p,
                         unsigned char *bin_r_p, unsigned char *bin_s_p) {
  ...
  unsigned char *serialized_pub_key =
      (unsigned char *)malloc(serialized_pub_key_size);
  ...
  sig_pubkey = ECDSA_do_sign_ex((const unsigned char *)serialized_pub_key,
                                serialized_pub_key_size, kinv, rp, ec_signing);
  if (sig_pubkey == NULL)
  ...
  // PublishKey(bin_x, size_bin_x, bin_y, size_bin_y, bin_r, bin_s, size_bin_r,
  // size_bin_s);
...
int8_t InitializeKeys(unsigned char *bin_x, unsigned char *bin_y,
                      unsigned char *bin_r, unsigned char *bin_s,
                      uint32_t size_bin) {
...
uint8_t createNewORAMInstance(uint32_t instance_id, uint32_t max_blocks,
                              uint32_t data_size, uint32_t stash_size,
                              uint32_t oblivious_flag,
                              uint32_t recursion_data_size,
                              int8_t recursion_levels, uint8_t oram_type,
                              uint8_t pZ) {
  ...
  if (oram_type == 0) {
    ...
    new_poram_instance->Create(instance_id, oram_type, pZ, max_blocks,
                               data_size, stash_size, oblivious_flag,
                               recursion_data_size, recursion_levels);
#ifdef DEBUG_ZT_ENCLAVE
    ...
    new_coram_instance->Create(instance_id, oram_type, pZ, max_blocks,
                               data_size, stash_size, oblivious_flag,
                               recursion_data_size, recursion_levels);
    return 0;
  }
...
uint32_t createNewLSORAMInstance(uint32_t key_size, uint32_t value_size,
                                 uint32_t num_blocks, uint8_t mode,
                                 uint8_t oblivious_type,
                                 uint8_t populate_flag) {
  LinearScan_ORAM *new_lsoram_instance =
      new LinearScan_ORAM(lsoram_instance_id, key_size, value_size, num_blocks,
...
int8_t LSORAMAccess(uint32_t instance_id, unsigned char *key, uint32_t key_size,
...
int8_t processLSORAMInsert(uint32_t instance_id, unsigned char *key,
                           uint32_t key_size, unsigned char *value,
                           uint32_t value_size) {
  LinearScan_ORAM *current_instance = lsoram_instances[instance_id];
  return (current_instance->insert(key, key_size, value, value_size));
}

int8_t processLSORAMFetch(uint32_t instance_id, unsigned char *key,
                          uint32_t key_size, unsigned char *value,
                          uint32_t value_size) {
  LinearScan_ORAM *current_instance = lsoram_instances[instance_id];
...
int8_t LSORAMEvict(uint32_t instance_id, unsigned char *key,
                   uint32_t key_size) {
  LinearScan_ORAM *current_instance = lsoram_instances[instance_id];
...
void accessInterface(uint32_t instance_id, uint8_t oram_type,
                     unsigned char *encrypted_request,
                     unsigned char *encrypted_response, unsigned char *tag_in,
                     unsigned char *tag_out, uint32_t encrypted_request_size,
                     uint32_t response_size, uint32_t tag_size) {
  ...
  status = sgx_rijndael128GCM_decrypt(
      (const sgx_aes_gcm_128bit_key_t *)SHARED_AES_KEY,
      (const uint8_t *)encrypted_request, encrypted_request_size,
      (uint8_t *)request, (const uint8_t *)HARDCODED_IV, IV_LENGTH, NULL, 0,
      (const sgx_aes_gcm_128bit_tag_t *)tag_in);
  /*
  if(status == SGX_SUCCESS)
          printf("Decrypt returned Success flag\n");
  else{
          if(status == SGX_ERROR_INVALID_PARAMETER)
                  printf("Decrypt returned SGX_ERROR_INVALID_PARAMETER Failure
  flag\n"); else printf("Decrypt returned another Failure flag\n");
  }
  */
  ...
  status = sgx_rijndael128GCM_encrypt(
      (const sgx_aes_gcm_128bit_key_t *)SHARED_AES_KEY, data_out, response_size,
      (uint8_t *)encrypted_response, (const uint8_t *)HARDCODED_IV, IV_LENGTH,
      NULL, 0, (sgx_aes_gcm_128bit_tag_t *)tag_out);
  /*
  if(status == SGX_SUCCESS)
          printf("Encrypt returned Success flag\n");
  else{
          if(status == SGX_ERROR_INVALID_PARAMETER)
                  printf("Encrypt returned SGX_ERROR_INVALID_PARAMETER Failure
  flag\n"); else printf("Encrypt returned another Failure flag\n");
  }
  */
...
void accessBulkReadInterface(uint32_t instance_id, uint8_t oram_type,
                             uint32_t no_of_requests,
                             unsigned char *encrypted_request,
  ...
  */
  ...
  // Encrypt Response
  status = sgx_rijndael128GCM_encrypt(
      (const sgx_aes_gcm_128bit_key_t *)SHARED_AES_KEY, response, response_size,
      (uint8_t *)encrypted_response, (const uint8_t *)HARDCODED_IV, IV_LENGTH,
      NULL, 0, (sgx_aes_gcm_128bit_tag_t *)tag_out);
...
*/
uint32_t parseInsertRequest(unsigned char *request, uint32_t request_size,
                            unsigned char **req_key, unsigned char **req_value,
                            uint32_t *req_key_size, uint32_t *req_value_size) {
...
/*
Input: encrypted_request, tag_in, request_size, tag_size, client_pubkey,
pubkey_size Output: aes_key, iv
*/
uint32_t DecryptRequest(unsigned char *encrypted_request,
                        unsigned char **request, uint32_t request_size,
                        unsigned char *tag_in, uint32_t tag_size,
                        unsigned char *client_pubkey, uint32_t pubkey_size_x,
                        uint32_t pubkey_size_y, unsigned char *aes_key,
                        unsigned char *iv) {
  ...
  */
  ...
  secret_len = ECDH_compute_key(secret, secret_len,
                                EC_KEY_get0_public_key(client_public_key),
  ...
  *request = (unsigned char *)malloc(request_size);
  ...
  status = sgx_rijndael128GCM_decrypt((const sgx_aes_gcm_128bit_key_t *)aes_key,
                                      ...
                                      request_size, (uint8_t *)*request,
                                      (const uint8_t *)iv, IV_LENGTH, NULL, 0,
                                      (const sgx_aes_gcm_128bit_tag_t *)tag_in);
...
uint32_t EncryptResponse(unsigned char *response, uint32_t response_size,
  ...
  sgx_status_t status;
  status = sgx_rijndael128GCM_encrypt(
      (const sgx_aes_gcm_128bit_key_t *)aes_key, response, response_size,
...
int8_t LSORAMFetch(uint32_t instance_id, unsigned char *encrypted_request,
                   ...
                   uint32_t response_size, unsigned char *tag_in,
                   unsigned char *tag_out, uint32_t tag_size,
  ...
  // DecryptRequest
  DecryptRequest(encrypted_request, &request, request_size, tag_in, tag_size,
                 client_pubkey, pubkey_size_x, pubkey_size_y, aes_key, iv);

  parseFetchRequest(request, request_size, &req_key);

  // LSORAMFetch()
  processLSORAMFetch(instance_id, req_key, request_size, response,
                     response_size);

  // EncryptResponse
  EncryptResponse(response, response_size, aes_key, iv, encrypted_response,
                  tag_out);
  free(response);
}

int8_t LSORAMInsert(uint32_t instance_id, unsigned char *encrypted_request,
                    uint32_t request_size, unsigned char *tag_in,
                    uint32_t tag_size, unsigned char *client_pubkey,
                    uint32_t pubkey_size, uint32_t pubkey_size_x,
  ...
  parseInsertRequest(request, request_size, &req_key, &req_value, &req_key_size,
                     &req_value_size);

  // LSORAMInsert()
  processLSORAMInsert(instance_id, req_key, req_key_size, req_value,
                      req_value_size);
}

int8_t LSORAMInsert_pt(uint32_t instance_id, unsigned char *key,
                       uint32_t key_size, unsigned char *value,
                       uint32_t value_size) {
...
int8_t HSORAMFetch(uint32_t lsoram_iid, uint32_t oram_iid, uint8_t oram_type,
  ...
  unsigned char *req_key, *request;
  // Hacky here because Oblivious functions currentyly work at 8 byte
  // granularity So we convert from 8 byte back to 4 byte index into ORAM
  // scheme.
  ...
  // LSORAMFetch()
  processLSORAMFetch(lsoram_iid, req_key, request_size,
                     (unsigned char *)&oram_index_t, sizeof(uint64_t));
...
int8_t HSORAMInsert(uint32_t lsoram_iid, uint32_t oram_iid, uint8_t oram_type,
                    uint64_t oram_index_p, unsigned char *encrypted_request,
                    uint32_t request_size, unsigned char *tag_in,
                    uint32_t tag_size, unsigned char *client_pubkey,
                    uint32_t pubkey_size, uint32_t pubkey_size_x,
                    uint32_t pubkey_size_y) {
  ...
  // LSORAMInsert()
  processLSORAMInsert(lsoram_iid, req_key, req_key_size,
                      (unsigned char *)&oram_index_t, sizeof(uint64_t));
...
/*
uint32_t LSORAMAccess_Handler(uint32_t instance_id, unsigned char
*encrypted_request, unsigned char *encrypted_response, unsigned char *tag_in,
unsigned char* tag_out, uint32_t request_size, uint32_t response_size, uint32_t
tag_size, unsigned char *client_pubkey, uint32_t pubkey_size){
  ...
  //    Returns a 32 byte secret
  secret_len = ECDH_compute_key(secret, secret_len,
EC_KEY_get0_public_key(client_public_key), sgx_EC_key_pair, NULL);
...
aes_key, (const uint8_t *) encrypted_request, request_size, (uint8_t *) request,
(const uint8_t *) iv, IV_LENGTH, NULL, 0, (const sgx_aes_gcm_128bit_tag_t*)
tag_in);
...
aes_key, response, response_size, (uint8_t *) encrypted_response, (const uint8_t
*) iv, IV_LENGTH, NULL, 0, (sgx_aes_gcm_128bit_tag_t *) tag_out);
