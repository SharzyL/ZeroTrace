...
ORAMTree::~ORAMTree() { free(aes_key); }
...
void ORAMTree::verifyPath(unsigned char *path_array, unsigned char *path_hash,
                          uint32_t leaf, uint32_t D, uint32_t block_size,
                          uint8_t level) {
  ...
  for (i = D - 1; i >= 0; i--) {
    if (i == (D - 1)) {
      ...
      sgx_sha256_msg(path_array_iter, (block_size * Z),
                     (sgx_sha256_hash_t *)child);
      ...
      if (temp % 2 == 0) {
        cmp1 =
            memcmp((uint8_t *)child, (uint8_t *)lchild_retrieved, HASH_LENGTH);
      } else {
        cmp1 =
            memcmp((uint8_t *)child, (uint8_t *)rchild_retrieved, HASH_LENGTH);
      }
      ...
      path_array_iter += (block_size * Z);
      sgx_sha256_update((uint8_t *)lchild_retrieved, SGX_SHA256_HASH_SIZE,
                        p_sha_handle);
      sgx_sha256_update((uint8_t *)rchild_retrieved, SGX_SHA256_HASH_SIZE,
                        p_sha_handle);
      ...
      path_hash_iter += HASH_LENGTH;
      // Test if retrieved merkle_root_hash of tree matches internally stored
      // merkle_root_hash If retrieved matches internal, then computed
      // merkle_root_hash should match as well for free.

      cmp = memcmp((uint8_t *)parent_hash_retrieved,
                   (uint8_t *)merkle_root_hash_level[level], HASH_LENGTH);
      ...
      if (temp % 2 == 0)
        cmp = memcmp((uint8_t *)lchild_retrieved, (uint8_t *)parent_hash,
                     HASH_LENGTH);
      else
        cmp = memcmp((uint8_t *)rchild_retrieved, (uint8_t *)parent_hash,
                     HASH_LENGTH);
    }
...
void ORAMTree::decryptPath(unsigned char *path_array,
                           unsigned char *decrypted_path_array,
                           uint32_t num_of_blocks_on_path, uint32_t data_size) {
...
void ORAMTree::encryptPath(unsigned char *path_array,
                           unsigned char *encrypted_path_array,
                           uint32_t num_of_blocks_on_path, uint32_t data_size) {
  unsigned char *path_iter = path_array;
...
uint32_t *ORAMTree::BuildTreeLevel(uint8_t level, uint32_t *prev_pmap) {
  ...
  uint32_t pD_temp =
      ceil((double)max_blocks_level[level] / (double)util_divisor);
  ...
  printf("\n\nBuildTreeLevel,\nLevel : %d, Params - D = %d, N = %d, treeSize = "
         "%d, x = %d\n",
  ...
  uint32_t *posmap_l =
      (uint32_t *)malloc(max_blocks_level[level] * sizeof(uint32_t));
  if (posmap_l == NULL) {
  ...
  printf("Posmap Size = %f MB\n",
         float(max_blocks_level[level] * sizeof(uint32_t)) /
             (float(1024 * 1024)));
  for (uint8_t jk = 0; jk < recursion_levels; jk++) {
    printf("real_max_blocks_level[%d] = %d \n", jk, real_max_blocks_level[jk]);
  }
  printf("\n");
  printf("pN = %d, level = %d, real_max_blocks_level[level] = %d, "
         "blocks_per_bucket_in_ll = %d\n",
  ...
  for (uint32_t i = pN; i <= ptreeSize; i++) {
    ...
    for (uint8_t q = 0; q < blocks_in_this_bucket; q++) {
      ...
      if (level < recursion_levels - 1 && level > 0) {
        temp.blocks[q].fill_recursion_data(&(prev_pmap[(label)*x]),
                                           recursion_data_size);
      }
    ...
    // TODO: No need to create and release memory for serialized bucket again
    // and again
    unsigned char *serialized_bucket = temp.serialize(tdata_size);
    uint8_t ret;

    // Hash / Integrity Tree
    sgx_sha256_msg(serialized_bucket, block_size * Z,
                   (sgx_sha256_hash_t *)&(current_bucket_hash));

    // Upload Bucket
    uploadBucket_OCALL(&ret, instance_id, oram_type, serialized_bucket,
                       Z * block_size, i,
                       (unsigned char *)&(current_bucket_hash), HASH_LENGTH,
                       block_size, level);
  ...
  for (uint32_t i = pN - 1; i >= 1; i--) {
    ...
    // Hash
    build_fetchChildHash(instance_id, oram_type, i * 2, i * 2 + 1, hash_lchild,
                         hash_rchild, HASH_LENGTH, level);
    sgx_sha_state_handle_t p_sha_handle;
    sgx_sha256_init(&p_sha_handle);
    ...
    sgx_sha256_get_hash(p_sha_handle,
                        (sgx_sha256_hash_t *)merkle_root_hash_level[level]);
    sgx_sha256_close(p_sha_handle);

    // Upload Bucket
    uploadBucket_OCALL(&ret, instance_id, oram_type, serialized_bucket,
                       Z * block_size, i,
                       (unsigned char *)&(merkle_root_hash_level[level]),
                       HASH_LENGTH, block_size, level);
  ...
  for (uint32_t i = 1; i < ptreeSize; i++) {
    downloadBucket_OCALL(&ret, instance_id, oram_type, serialized_bucket,
                         Z * block_size, i, temp_hash, HASH_LENGTH, block_size,
                         level);
    ...
    for (uint8_t j = 0; j < Z; j++) {
      ...
      uint32_t *data_iter =
          (uint32_t *)(bucket_iter + ADDITIONAL_METADATA_SIZE);
...
void ORAMTree::BuildTreeRecursive(uint8_t level, uint32_t *prev_pmap) {
  if (level == 0) {
    ...
    if (recursion_levels == 1) {
      ...
      posmap_l =
          (uint32_t *)malloc(real_max_blocks_level[level] * sizeof(uint32_t));
      memcpy(posmap_l, prev_pmap,
             real_max_blocks_level[level] * sizeof(uint32_t));
      D_level[level] = 0;
...
uint8_t rt;
downloadObject(&rt, bucket_array, Z*block_size, i, hash,
HASH_LENGTH,level,D_level[level]); Bucket temp2(bucket_array,data_size);
...
void ORAMTree::Initialize() {
  ...
  for (uint32_t i = 0; i < recursion_levels; i++) {
    ...
    if (i != recursion_levels - 1 && recursion_levels != 1) {
      ...
      if (oblivious_flag) {
        printf(
            "Correct recursive stash setup call was invoked, gn = %d \n, gn");
  ...
  // Allocate encrypted_path and decrypted_path to be the largest path sizes the
  // ORAM would ever need So that we dont have to have costly malloc and free
  // within access() Since ZT is currently single threaded, these are shared
  // across all ORAM instances Will have to redesign these to be comoponents of
  // the ORAM_Instance class in a multi-threaded setting.
  // PerformMemoryAllocations()

  uint64_t largest_path_size =
      Z * (data_size + ADDITIONAL_METADATA_SIZE) * (d_largest);
  // printf("Z=%d, data_size=%d, d_largest=%d, Largest_path_size = %ld\n", Z,
  // data_size, d_largest, largest_path_size);
  ...
  serialized_result_block =
      (unsigned char *)malloc(data_size + ADDITIONAL_METADATA_SIZE);
}

/*
uint32_t ORAMTree::savePosmap(unsigned char *posmap_serialized, uint32_t
posmap_size) { uint32_t real_pmap_size; if(recursive_posmap){ real_pmap_size =
max_blocks_level[0]*16;
...
*/
/*
uint32_t saveState(unsigned char *posmap_serialized, uint32_t posmap_size,
unsigned char* stash_serialized, uint32_t stash_size) {
        //ASIDE : Test if SGX will fail if posmap_serialized returned a pointer
to the posmap in PRM memcpy(posmap_serialized, posmap, posmap_size); uint32_t
stash_size_returned = stash.saveStash(stash_serialized, -1); return
stash_size_returned;
}
uint32_t restoreState(uint32_t* posmap_stored, uint32_t posmap_size, uint32_t
*stash_serialized, uint32_t stash_size)
{
...
void ORAMTree::uploadPath(uint32_t leaf, unsigned char *path,
                          uint64_t path_size, unsigned char *path_hash,
                          uint64_t path_hash_size, uint8_t level) {
  uint32_t d = D_level[level];
  uint8_t ret;

#ifdef ENCRYPTION_ON
  uploadPath_OCALL(&ret, instance_id, oram_type, encrypted_path, path_size,
                   leaf, path_hash, path_hash_size, level, d);
#else
  uploadPath_OCALL(&ret, instance_id, oram_type, decrypted_path, path_size,
                   leaf, path_hash, path_hash_size, level, d);
#endif
}

// For non-recursive level = 0
unsigned char *ORAMTree::downloadPath(uint32_t leaf, unsigned char *path_hash,
                                      uint8_t level) {
  uint32_t temp = leaf;
  uint8_t rt;
  uint32_t tdata_size;
...
#else
  downloadPath_OCALL(&rt, instance_id, oram_type, fetched_path_array, path_size,
                     leaf, path_hash, path_hash_size, level, d);
#endif

#ifndef PASSIVE_ADVERSARY
  verifyPath(fetched_path_array, path_hash, leaf, d,
             tdata_size + ADDITIONAL_METADATA_SIZE, level);
#endif
...
void ORAMTree::createNewPathHash(unsigned char *path_ptr,
                                 unsigned char *old_path_hash,
                                 unsigned char *new_path_hash, uint32_t leaf,
                                 uint32_t block_size, uint8_t level) {
  uint32_t d = D_level[level];
  ...
  for (uint8_t i = 0; i < d + 1; i++) {
    if (i == 0) {
      sgx_sha256_msg(path_ptr, (block_size * Z),
                     (sgx_sha256_hash_t *)new_path_hash);
...
void ORAMTree::PushBlocksFromPathIntoStash(
    unsigned char *decrypted_path_ptr, uint8_t level, uint32_t data_size,
    uint32_t block_size, uint32_t id, uint32_t position_in_id, uint32_t leaf,
    uint32_t *nextLeaf, uint32_t newleaf, uint32_t sampledLeaf,
    int32_t newleaf_nextlevel) {
  uint32_t d = D_level[level];
  uint32_t i;
#ifdef ACCESS_DEBUG
  ...
  for (i = 0; i < (Z * (d)); i++) {
    ...
    if (oblivious_flag) {
      recursive_stash[level].pass_insert(decrypted_path_ptr,
                                         isBlockDummy(decrypted_path_ptr, gN));
...
void ORAMTree::OAssignNewLabelToBlock(uint32_t id, uint32_t position_in_id,
                                      uint8_t level, uint32_t newleaf,
                                      uint32_t newleaf_nextlevel,
                                      uint32_t *nextLeaf) {
  uint32_t k;
  nodev2 *listptr_t;
  ...
  for (k = 0; k < stash_size; k++) {
    ...
    flag1 = ((getId(listptr_t->serialized_block) == id) &&
             (!isBlockDummy(listptr_t->serialized_block, gN)));
    oassign_newlabel(getTreeLabelPtr(listptr_t->serialized_block), newleaf,
                     flag1);
    ...
    if (level != recursion_levels && recursion_levels != -1) {
      for (uint8_t p = 0; p < x; p++) {
        ...
        ofix_recursion(&(listptr_t->serialized_block[24 + p * 4]), flag2,
                       newleaf_nextlevel, nextLeaf);
        /*
        #ifdef ACCESS_DEBUG
          if(getId(listptr_t->serialized_block) == id) {
            for(uint8_t p = 0;p< recursion_block_size/4;p++) {
...
uint32_t ORAMTree::FillResultBlock(uint32_t id, unsigned char *result_data,
                                   uint32_t block_size) {
...
// newleaf_nextleaf,arr_blocks,  path_hash,level,D_level[level],N_level[level]);

/*
uint32_t ORAMTree::access_oram_level(char opType, uint32_t leaf, uint32_t id,
uint32_t position_in_id, uint32_t level, uint32_t newleaf,uint32_t
newleaf_nextleaf, unsigned char *data_in,  unsigned char *data_out)
{
    ...
    decrypted_path = ReadBucketsFromPath(leaf + N_level[level], path_hash,
level);

    switch(oram_controller){
        case PATHORAM : return_value = PathORAM_Access(opType, id,
position_in_id,leaf, newleaf, newleaf_nextleaf,decrypted_path,
                                path_hash,level,D_level[level],N_level[level],
data_in, data_out); break; case CIRCUITORAM: return_value =
CircuitORAM_Access(opType, id, position_in_id, leaf, newleaf,
newleaf_nextleaf,decrypted_path, path_hash,level,D_level[level],N_level[level],
data_in, data_out); break;
    }

    return return_value;
}
*/
// if target[i] != _|_, then one block should be moved from path[i] to
// path[target[i]]
...
void ORAMTree::print_stash_count(uint32_t level, uint32_t nlevel) {
  ...
  if (recursion_levels > 0) {
    ...
    recursive_stash[level].displayStashContents(
        nlevel, level != (recursion_levels - 1));
  }
}

void ORAMTree::showPath(unsigned char *decrypted_path, uint8_t Z, uint32_t d,
                        uint32_t data_size) {
  ...
  if (data_size == recursion_data_size) {
    for (uint32_t i = 0; i < num_of_blocks_on_path; i++) {
      ...
      printf("(%d,%d) :", getId(decrypted_path_iter),
             getTreeLabel(decrypted_path_iter));
      uint32_t no = (data_size) / sizeof(uint32_t);
      uint32_t *data_iter =
          (uint32_t *)(decrypted_path_iter + ADDITIONAL_METADATA_SIZE);
      unsigned char *data_ptr =
          (unsigned char *)(decrypted_path_iter + ADDITIONAL_METADATA_SIZE);
...
void ORAMTree::showPath_reverse(unsigned char *decrypted_path, uint8_t Z,
                                uint32_t d, uint32_t data_size,
                                uint32_t bucket_id_of_leaf) {
  printf("\n\nshowPath_reverse (Root to leaf): \n");
  uint32_t block_size = data_size + ADDITIONAL_METADATA_SIZE;
  unsigned char *decrypted_path_iter =
      decrypted_path + ((uint64_t)((Z * (d - 1))) * uint64_t(block_size));
    ...
    for (uint32_t i = 0; i < d; i++) {
      ...
      for (uint32_t j = 0; j < Z; j++) {
        ...
        uint32_t *data_iter =
            (uint32_t *)(bucket_iter + ADDITIONAL_METADATA_SIZE);
        unsigned char *data_ptr =
            (unsigned char *)(bucket_iter + ADDITIONAL_METADATA_SIZE);
...
void ORAMTree::SetParams(uint32_t s_instance_id, uint8_t s_oram_type,
                         uint8_t pZ, uint32_t s_max_blocks,
                         uint32_t s_data_size, uint32_t s_stash_size,
                         uint32_t oblivious, uint32_t s_recursion_data_size,
                         uint8_t precursion_levels) {
  ...
  real_max_blocks_level =
      (uint64_t *)malloc((recursion_levels) * sizeof(uint64_t));
  ...
  while (lev >= 0) {
    real_max_blocks_level[lev] =
        ceil((double)real_max_blocks_level[lev + 1] / (double)x);
    lev--;
  }
  ...
  merkle_root_hash_level = (sgx_sha256_hash_t *)malloc(
      (max_recursion_level_index + 1) * sizeof(sgx_sha256_hash_t));
}
