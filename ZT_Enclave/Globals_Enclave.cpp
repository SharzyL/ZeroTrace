...
void displaySerializedBlock(unsigned char *serialized_result_block,
  ...
  printf("Fetched Result_block : (%d,%d) : \n", getId(serialized_result_block),
         getTreeLabel(serialized_result_block));
...
void aes_dec_serialized(unsigned char *encrypted_block, uint32_t data_size,
  ...
  ret = sgx_aes_ctr_decrypt((const sgx_aes_ctr_128bit_key_t *)aes_key,
                            encrypted_block_ptr, ciphertext_size, ctr,
                            ctr_inc_bits, decrypted_block_ptr);
  free(ctr);
}
...
void aes_enc_serialized(unsigned char *decrypted_block, uint32_t data_size,
  ...
  ret = sgx_aes_ctr_encrypt((const sgx_aes_ctr_128bit_key_t *)aes_key,
                            decrypted_block_ptr, input_size, ctr, ctr_inc_bits,
                            encrypted_block_ptr);
