...
void oarray_search2(uint32_t *array, uint32_t loc, uint32_t *leaf,
...
uint32_t firstkbits(uint32_t n, uint32_t k) { return (n >> (32 - k)); }

void CircuitORAM::Initialize(uint32_t instance_id, uint8_t oram_type,
                             uint8_t pZ, uint32_t pmax_blocks,
                             uint32_t pdata_size, uint32_t pstash_size,
                             uint32_t poblivious_flag,
                             uint32_t precursion_data_size,
                             uint8_t precursion_levels) {
#ifdef BUILDTREE_DEBUG
  printf("In CircuitORAM::Initialize, Started Initialize\n");
#endif

  ORAMTree::SetParams(instance_id, oram_type, pZ, pmax_blocks, pdata_size,
                      pstash_size, poblivious_flag, precursion_data_size,
                      precursion_levels);
  ORAMTree::Initialize();
  ...
  // deepest and target buffers also treat stash as level 0 of the logical path,
  // hence +1
...
/*
printf("Testing Deterministic Reverse Lexicographic Eviction leaf label
...
uint32_t *CircuitORAM::prepare_target(uint32_t leaf,
                                      unsigned char *serialized_path,
                                      uint32_t block_size, uint32_t level,
                                      uint32_t *deepest,
  ...
  for (i = D; i >= 0; i--) {
    ...
    if (i == 0) {
      ...
      for (k = 0; k < Z; k++) {
        ...
        pt_set_target_position(&(target_position[i]), k, &target_position_set,
                               flag_stp);
      ...
      uint32_t flag_pt =
          (((dest == -1 && flag_empty_slot)) && (deepest[i] != -1));
...
// Root - to - leaf linear scan , NOTE: serialized_path from LS is filled in
// leaf to root Creates deepest[] , where deepest[i] stores the source level of
// the deepest block in path[0,...,i-1] that can reside in path[i]
uint32_t *CircuitORAM::prepare_deepest(uint32_t leaf,
                                       unsigned char *serialized_path,
                                       uint32_t block_size, uint32_t level,
  ...
  int32_t i;
  uint32_t k;
  unsigned char *serialized_path_ptr =
      serialized_path + (Z * D - 1) * (block_size);
  ...
  for (i = 0; i <= D; i++) {
    if (i == 0) {
      ...
      for (k = 0; k < stash_size; k++) {
        ...
        // level_in_path keeps track of which level in the path will this block
        // fit into
        uint32_t level_in_path = D, position_in_path = 0;
        for (uint32_t j = 0; j < D; j++) {
          uint32_t flag_deepest =
              (l1 == l2) && (!isBlockDummy(listptr_t->serialized_block, gN) &&
                             l1 > local_deepest);
          ...
          if (!isBlockDummy(listptr_t->serialized_block, gN)) {
            printf("\t(%d, %d) - local_deepest = %d, level_in_path = %d\n", l1,
                   l2, local_deepest, level_in_path);
          }
#endif
        ...
        oset_value((uint32_t *)&(deepest_position[i]), k,
                   position_in_path > goal);
        oset_goal_source(i, position_in_path,
                         (int32_t)position_in_path > goal &&
                             position_in_path > i,
                         &src, &goal);
      ...
      for (k = 0; k < Z; k++) {
        ...
        for (uint32_t j = 0; j < D - i; j++) {
          uint32_t flag_deepest =
              (l1 == l2) && !dummy_flag && l1 > local_deepest;
          oset_value(&local_deepest, l1, flag_deepest);
          oset_value((uint32_t *)&(deepest_position[i]), k, flag_deepest);
          oset_value(&position_in_path, level_in_path, flag_deepest);
          ...
          if (!dummy_flag)
            printf(
                "\t(%d, %d) - local_deepest = %d, deepest_position[%d] = %d\n",
...
void CircuitORAM::EvictOnceFast(uint32_t *deepest, uint32_t *target,
                                int32_t *deepest_position,
                                int32_t *target_position,
                                unsigned char *serialized_path,
                                unsigned char *path_hash, uint32_t level,
  ...
  unsigned char *serialized_path_ptr =
      serialized_path + (Z * dlevel - 1) * (tblock_size);
  ...
  for (uint32_t i = 0; i <= (dlevel); i++) {
    // Oblivious function for Setting Write flag, hold , dest and moving
    // held_block to block_write
    uint32_t flag_1 = ((hold != -1) && (i == dest));
    omove_serialized_block(serialized_block_write, serialized_block_hold,
                           tdata_size, flag_1);
    ...
    if (i == 0) {
      ...
      for (uint32_t k = 0; k < stash_size; k++) {
        ...
        uint32_t flag_hold = (k == deepest_position[i] && (target[i] != -1) &&
                              (!isBlockDummy(stash_block, gN)));

#ifdef DEBUG_EFO
        if (flag_hold)
          printf("Held block ID = %d, treelabel = %d\n", getId(stash_block),
                 getTreeLabel(stash_block));
#endif

        omove_serialized_block(serialized_block_hold, stash_block, tdata_size,
                               flag_hold);
      ...
      for (uint32_t k = 0; k < Z; k++) {
        uint32_t flag_hold =
            (k == deepest_position[i]) && (hold == -1) && (target[i] != -1);
        omove_serialized_block(serialized_block_hold, serialized_path_ptr,
                               tdata_size, flag_hold);
#ifdef DEBUG_EFO
        if (flag_hold)
          printf("Held Block in bucket, ID = %d, treelabel = %d\n",
                 getId(serialized_block_hold),
                 getTreeLabel(serialized_block_hold));
#endif
    ...
    if (i != 0) {
      ...
      for (int32_t k = (Z - 1); k >= 0; k--) {
        ...
        omove_serialized_block(serialized_path_ptr2, serialized_block_write,
                               tdata_size, flag_w);

#ifdef DEBUG_EFO
        printf("i = %d, k = %d, write_flag = %d, flag_w = %d, writeblock_id = "
               "%d, writeblock_treelabel = %d\n",
               i, k, write_flag, flag_w, getId(serialized_block_write),
               getTreeLabel(serialized_block_write));
        if (flag_w)
          printf("serialized_path_ptr2 : ID = %d, treeLabel = %d\n",
        ...
        // printf("EVERY PATHBLOCK: serialized_path_ptr2 : ID = %d, treeLabel =
        // %d\n",getId(serialized_path_ptr2),
        // getTreeLabel(serialized_path_ptr2));
...
void CircuitORAM::CircuitORAM_FetchBlock(uint32_t *return_value, uint32_t leaf,
                                         uint32_t newleaf, char opType,
                                         uint32_t id, uint32_t position_in_id,
                                         uint32_t newleaf_nextlevel,
                                         uint32_t level, unsigned char *data_in,
  ...
  for (i = 0; i < (Z * (dlevel)); i++) {
    if (oblivious_flag) {
      ...
      omove_serialized_block(serialized_result_block, decrypted_path_ptr,
                             data_size, block_id == id);
  ...
  if (oblivious_flag) {
    for (k = 0; k < stash_size; k++) {
      ...
      omove_serialized_block(serialized_result_block, stash_block, data_size,
                             flag_move);
      listptr_t = listptr_t->next;
    }
  ...
  // Obliviously read to data_out or obliviously write data_in to
  // serialized_result_block before inserting it into stash.
...
// Path Integrity Module
  ...
  createNewPathHash(path_ptr, path_hash, new_path_hash, leaf, tblock_size,
                    level);
#endif
...
#ifdef ENCRYPTION_ON
  uploadPath(leaf, encrypted_path, path_size, new_path_hash, new_path_hash_size,
             level);
#else
  uploadPath(leaf, decrypted_path, path_size, new_path_hash, new_path_hash_size,
             level);
#endif
  ...
  if (oblivious_flag) {
    recursive_stash[level].pass_insert(
        serialized_result_block, isBlockDummy(serialized_result_block, gN));
  } else {
    recursive_stash[level].insert(serialized_result_block);
  }

  if (level == recursion_levels) {
    recursive_stash[level].PerformAccessOperation(opType, id, newleaf, data_in,
                                                  data_out);
  }
}

uint32_t CircuitORAM::access_oram_level(char opType, uint32_t leaf, uint32_t id,
                                        uint32_t position_in_id, uint32_t level,
                                        uint32_t newleaf,
                                        uint32_t newleaf_nextleaf,
  ...
  return_value = CircuitORAM_Access(opType, id, position_in_id, leaf, newleaf,
                                    newleaf_nextleaf, decrypted_path, path_hash,
                                    level, data_in, data_out);
  return return_value;
}
...
void CircuitORAM::EvictionRoutine(uint32_t leaf, uint32_t level) {
  ...
  deepest = prepare_deepest(leaf_left, eviction_path_left, tblock_size, level,
                            deepest_position);
  ...
  for (i = 0; i <= dlevel; i++) {
    printf("Level : %d, %d, deepest_position = %d\n", i, deepest[i],
           (int32_t)(deepest_position[i]));
  }
#endif

  // prepare_target Create target[]
  target = prepare_target(leaf_left, eviction_path_left, tblock_size, level,
                          deepest, target_position);

#ifdef ACCESS_CORAM_META_DEBUG
  printf("Printing Target : \n");
  for (i = 0; i <= dlevel; i++) {
    printf("%d , target_position = %d\n", target[i],
           (int32_t)(target_position[i]));
  }
#endif

  EvictOnceFast(deepest, target, deepest_position, target_position,
                eviction_path_left, path_hash, level, new_path_hash, leaf_left);
  ...
  uint32_t new_path_hash_size = HASH_LENGTH * D_level[level];
#ifdef ENCRYPTION_ON
  uploadPath(leaf_left, encrypted_path, path_size, new_path_hash,
             new_path_hash_size, level);
#else
  uploadPath(leaf_left, eviction_path_left, path_size, new_path_hash,
             new_path_hash_size, level);
#endif
  ...
  createNewPathHash(path_ptr, path_hash, new_path_hash, leaf_left, tblock_size,
                    level);
  /*
  for(i=0;i < ( Z * dlevel ); i++) {
    if(i%Z==0) {
      uint32_t p = i/Z + 1;
      addToNewPathHash(path_ptr, old_path_hash_iter, new_path_hash_trail,
  new_path_hash_iter,dlevel-p, leaf_temp_prev, tblock_size, level);
      leaf_temp_prev>>1;
  ...
  */
  ...
  deepest = prepare_deepest(leaf_right, eviction_path_right, tblock_size, level,
                            deepest_position);
  target = prepare_target(leaf_right, eviction_path_right, tblock_size, level,
                          deepest, target_position);
  ...
  EvictOnceFast(deepest, target, deepest_position, target_position,
                eviction_path_right, path_hash, level, new_path_hash,
                leaf_right);
#ifdef ACCESS_DEBUG
  ...
  createNewPathHash(path_ptr, path_hash, new_path_hash, leaf_right, tblock_size,
                    level);
  /*
  for(i=0;i < Z*dlevel; i++) {
    if(i%Z==0) {
...
#ifdef ENCRYPTION_ON
  uploadPath(leaf_right, encrypted_path, path_size, new_path_hash,
             new_path_hash_size, level);
#else
  uploadPath(leaf_right, eviction_path_right, path_size, new_path_hash,
             new_path_hash_size, level);
#endif
...
uint32_t CircuitORAM::CircuitORAM_Access(
    char opType, uint32_t id, uint32_t position_in_id, uint32_t leaf,
    uint32_t newleaf, uint32_t newleaf_nextlevel, unsigned char *decrypted_path,
    unsigned char *path_hash, uint32_t level, unsigned char *data_in,
    unsigned char *data_out) {
  ...
  CircuitORAM_FetchBlock(&return_value, leaf, newleaf, opType, id,
                         position_in_id, newleaf_nextlevel, level, data_in,
                         data_out);
...
uint32_t CircuitORAM::access(uint32_t id, int32_t position_in_id, char opType,
                             uint8_t level, unsigned char *data_in,
                             unsigned char *data_out,
                             uint32_t *prev_sampled_leaf) {
  uint32_t leaf = 0;
  uint32_t nextLeaf;
  uint32_t id_adj;
  uint32_t newleaf;
  ...
  if (recursion_levels == 1) {
    ...
    uint32_t newleaf =
        (N_level[0]) + (*((uint32_t *)random_value) % N_level[0]);
    ...
    CircuitORAM_Access(opType, id, -1, leaf, newleaf, -1, decrypted_path,
    ...
    newleaf = (N_level[level + 1] - 1) +
              (*((uint32_t *)random_value) % (N_level[level + 1]));
    *prev_sampled_leaf = newleaf;
...
#ifdef ACCESS_DEBUG
    printf("access : Level = %d: \n Requested_id = %d, Corresponding leaf from "
           "posmap = %d, Newleaf assigned = %d,\n\n",
    ...
    leaf = access(id_adj, position_in_id, opType, level - 1, data_in, data_out,
                  prev_sampled_leaf);
#ifdef ACCESS_DEBUG
    printf("access, Level = %d:  before access_oram_level : Block_id = %d, "
           "Newleaf = %d, Leaf from level = %d, Flag = %d\n",
           level, id, *prev_sampled_leaf, leaf, oblivious_flag);
#endif
    // ORAM ACCESS of recursion_levels is to fetch entire Data block, no
    // position_in_id, hence -1)
    access_oram_level(opType, leaf, id, -1, level, *prev_sampled_leaf, -1,
                      data_in, data_out);
    return 0;
  } else {
    id_adj = id / x;
    uint32_t nl_position_in_id = (level == 1) ? -1 : id % x;
    leaf = access(id_adj, nl_position_in_id, opType, level - 1, data_in,
                  data_out, prev_sampled_leaf);

    // sampling leafs for a level ahead
    sgx_read_rand((unsigned char *)random_value, ID_SIZE_IN_BYTES);
    newleaf_nextlevel = (N_level[level + 1]) +
                        (*((uint32_t *)random_value) % N_level[level + 1]);
    newleaf = *prev_sampled_leaf;
    *prev_sampled_leaf = newleaf_nextlevel;
#ifdef ACCESS_DEBUG
    printf("access, Level = %d :\n leaf = %d, block_id = %d, position_in_id = "
           "%d, newLeaf = %d, newleaf_nextlevel = %d\n",
           level, leaf, id, position_in_id, newleaf, newleaf_nextlevel);
#endif
    nextLeaf = access_oram_level(opType, leaf, id, position_in_id, level,
                                 newleaf, newleaf_nextlevel, data_in, data_out);
#ifdef ACCESS_DEBUG
...
/*
void CircuitORAM::Access_temp(uint32_t id, char opType, unsigned char* data_in,
unsigned char* data_out){ uint32_t prev_sampled_leaf=-1; access(id, -1, opType,
recursion_levels, data_in, data_out, &prev_sampled_leaf);
}
...
void CircuitORAM::Create(uint32_t instance_id, uint8_t oram_type, uint8_t pZ,
                         uint32_t pmax_blocks, uint32_t pdata_size,
                         uint32_t pstash_size, uint32_t poblivious_flag,
                         uint32_t precursion_data_size,
                         uint8_t precursion_levels) {
  Initialize(instance_id, oram_type, pZ, pmax_blocks, pdata_size, pstash_size,
             poblivious_flag, precursion_data_size, precursion_levels);
}

void CircuitORAM::Access(uint32_t id, char opType, unsigned char *data_in,
                         unsigned char *data_out) {
  uint32_t prev_sampled_leaf = -1;
  access(id, -1, opType, recursion_levels - 1, data_in, data_out,
         &prev_sampled_leaf);
}
