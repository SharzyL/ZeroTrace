...
void getParams(int argc, char *argv[]) {
  if (argc < min_expected_no_of_parameters) {
    ...
    printf(" <N> <No_of_requests> <key_size> <value_size> <0/1 = Store "
           "In-PRM/Outside-PRM> <0/1 = Access-Oblivious/Full-Oblivious> "
           "<Logfile>\n");
  }
...
int generateKeyValuePair(unsigned char *key, unsigned char *value,
...
void displayKeyValuePair(unsigned char *key, unsigned char *value,
                         uint32_t key_size, uint32_t value_size) {
  printf("<");
...
void client_LSORAM_Insert(uint32_t lsoram_iid, uint32_t oram_iid,
  ...
  uint32_t request_size = serializeLSORAMRequest(
      key, key_size, value, value_size, &serialized_request);

  encryptLSORAMRequest(ENCLAVE_PUBLIC_KEY, serialized_request, request_size,
                       &encrypted_request, &client_pubkey, &pubkey_size_x,
                       &pubkey_size_y, &ecdh_aes_key, &iv, &tag_in);
...
#ifdef HSORAM_MODE
  ZT_HSORAM_insert(lsoram_iid, oram_iid, HSORAM_ORAM_TYPE, oram_index++,
                   encrypted_request, request_size, tag_in, TAG_SIZE,
                   client_pubkey, pubkey_size_x, pubkey_size_y);
#else
  ZT_LSORAM_insert(lsoram_iid, encrypted_request, request_size, tag_in,
                   TAG_SIZE, client_pubkey, pubkey_size_x, pubkey_size_y);
#endif
...
void client_LSORAM_Fetch(uint32_t lsoram_iid, uint32_t oram_iid,
                         unsigned char *key, uint32_t key_size,
                         unsigned char *value, uint32_t value_size,
                         uint32_t req_ctr, double *gentime, double *processtime,
                         double *extracttime) {
  ...
  // uint32_t request_size = serializeLSORAMRequest(key, key_size,
  // encrypted_value, 0, &serialized_request);

  // Request is just key encrypted for fetch
  encryptLSORAMRequest(ENCLAVE_PUBLIC_KEY, key, key_size, &encrypted_request,
                       &client_pubkey, &pubkey_size_x, &pubkey_size_y,
                       &ecdh_aes_key, &iv, &tag_in);

  generate_request_stop = clock();
  generate_request_time = generate_request_stop - generate_request_start;
  // printf("Request Generate time = %f
...
#ifdef HSORAM_MODE
  ZT_HSORAM_fetch(lsoram_iid, oram_iid, HSORAM_ORAM_TYPE, encrypted_request,
                  key_size, value, value_size, tag_in, tag_out, TAG_SIZE,
                  client_pubkey, pubkey_size_x, pubkey_size_y);
#else
  ZT_LSORAM_fetch(lsoram_iid, encrypted_request, key_size, value, value_size,
                  tag_in, tag_out, TAG_SIZE, client_pubkey, pubkey_size_x,
                  pubkey_size_y);
#endif
...
int main(int argc, char *argv[]) {
...
#ifdef HSORAM_MODE
  zt_lsoram_id = ZT_New_LSORAM(NUM_BLOCKS, key_size, HSORAM_INDEX_SIZE,
                               store_mode, oblivious_mode, 1);
  zt_oram_id = ZT_New(HSORAM_MAX_BLOCKS, value_size, HSORAM_STASH_SIZE,
                      HSORAM_OBLIVIOUS_TYPE_ORAM, HSORAM_RECURSION_DATA_SIZE,
                      HSORAM_ORAM_TYPE, HSORAM_Z);
#else
  zt_lsoram_id = ZT_New_LSORAM(NUM_BLOCKS, key_size, value_size, store_mode,
                               oblivious_mode, 1);
#endif
  ...
  for (int i = 0; i < NUM_BLOCKS; i++) {
    ...
    if (store_mode == 1) {
      ...
      // client_LSORAM_Insert(zt_lsoram_id, zt_oram_id, key, key_size, value,
      // value_size);
    }
    inserts_stop = clock();
  ...
  for (int i = 0; i < requestlength; i++) {
    ...
    fetches_start = clock();
    client_LSORAM_Fetch(zt_lsoram_id, zt_oram_id, key, key_size,
                        encrypted_value_returned, value_size, i, gentime,
                        processtime, extracttime);
    fetches_stop = clock();
...
// If in Correctness Test Mode, check that the returned value is correct.
    ...
    val_returned.assign((const char *)encrypted_value_returned,
                        (size_t)value_size);
  ...
  fprintf(fptr, "%d, %f, %f, %f, %f, %f, %f, %ld, %ld\n", NUM_BLOCKS,
          gentime_avg, gentime_std, processtime_avg, processtime_std,
          extracttime_avg, extracttime_std, request_size, response_size);
  fclose(fptr);
