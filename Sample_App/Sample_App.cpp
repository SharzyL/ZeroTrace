...
void getParams(int argc, char *argv[]) {
  ...
  if (argc != NUM_EXPECTED_PARAMS) {
    printf("Command line parameters error, received: %d, expected :%d\n", argc,
           NUM_EXPECTED_PARAMS);
    printf(
        " <N> <No_of_requests> <Stash_size> <Data_block_size> "
        "<\"resume\"/\"new\"> <\"memory\"/\"hdd\"> <0/1 = "
        "Non-oblivious/Oblivious> <Recursion_block_size> "
        "<\"auto\"/\"path\"/\"circuit\"> <Z> <bulk_batch_size> <LogFile>\n\n");
    exit(0);
  }
  ...
  std::string qfile_name =
      "ZT_" + std::to_string(MAX_BLOCKS) + "_" + std::to_string(DATA_SIZE);
...
int main(int argc, char *argv[]) {
  ...
  uint32_t *rs =
      reqsource.GenerateRandomSequence(REQUEST_LENGTH, MAX_BLOCKS - 1);
  uint32_t i = 0;
  ...
  if (bulk_batch_size == 0) {
    ...
    for (i = 0; i < REQUEST_LENGTH; i++) {
      ...
      ZT_Access(instance_id, ORAM_TYPE, encrypted_request, encrypted_response,
                tag_in, tag_out, encrypted_request_size, response_size,
                TAG_SIZE);
    ...
    for (i = 0; i < REQUEST_LENGTH / bulk_batch_size; i++) {
      ...
      generate_request_start = clock();
      encryptBulkReadRequest(rs, req_counter, bulk_batch_size,
                             encrypted_request, tag_in, encrypted_request_size);
      generate_request_stop = clock();

      // decryptBulkReadRequest(bulk_batch_size, encrypted_request, tag_in,
      // encrypted_request_size);

      // Process Request:
      process_request_start = clock();
      ZT_Bulk_Read(instance_id, ORAM_TYPE, bulk_batch_size, encrypted_request,
                   encrypted_response, tag_in, tag_out, encrypted_request_size,
