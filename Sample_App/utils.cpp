...
uint32_t *RandomRequestSource::GenerateRandomSequence(uint32_t length,
...
void prepareDataBlock(unsigned char *datablock, uint32_t index,
                      uint32_t data_size) {
...
int checkFetchedDataBlock(unsigned char *datablock, uint32_t index,
                          uint32_t data_size) {
  unsigned char *data_ptr = datablock;
...
int encryptRequest(int request_id, char op_type, unsigned char *data,
                   uint32_t data_size, unsigned char *encrypted_request,
                   unsigned char *tag, uint32_t request_size) {
  int encrypted_request_size;
  // Sample IV;
  unsigned char *iv = (unsigned char *)malloc(IV_LENGTH);
  // 1 from op_type
  unsigned char *serialized_request =
      (unsigned char *)malloc(1 + ID_SIZE_IN_BYTES + data_size);
  serializeRequest(request_id, op_type, data, data_size, serialized_request);

  encrypted_request_size = AES_GCM_128_encrypt(
      serialized_request, request_size, NULL, 0,
      (unsigned char *)SHARED_AES_KEY, (unsigned char *)HARDCODED_IV, IV_LENGTH,
...
int serializedSizeLSORAMRequest(uint32_t key_size, uint32_t value_size) {
  uint32_t serialized_request_size =
...
int32_t serializeLSORAMRequest(unsigned char *key, uint32_t key_size,
  ...
  uint32_t serialized_request_size =
      serializedSizeLSORAMRequest(key_size, value_size);
...
Outputs: instantiates and populates:
          client_pubkey, aes_key (from target_pubkey and generated client_pubkey
ECDH) iv, encrypted request and tag for the request
*/
int encryptLSORAMRequest(EC_KEY *target_public_key,
                         ...
                         uint32_t *pubkey_size_y, unsigned char **ecdh_aes_key,
  ...
  */
  ...
  uint32_t field_size =
      EC_GROUP_get_degree(EC_KEY_get0_group(target_public_key));
  ...
  secret_len = ECDH_compute_key(secret, secret_len,
                                EC_KEY_get0_public_key(target_public_key),
                                ephemeral_key, NULL);
  ...
  */
  encrypted_request_size = AES_GCM_128_encrypt(
      serialized_request, request_size, NULL, 0, (unsigned char *)*ecdh_aes_key,
      (unsigned char *)*iv, IV_LENGTH, *encrypted_request, *tag);
  ...
  encrypted_request_size); printf("Request bytes after encrypting: \n");
...
int decryptLSORAMResponse(unsigned char *encrypted_response,
                          uint32_t response_size, unsigned char *tag,
                          unsigned char *aes_key, unsigned char *iv,
                          unsigned char **response) {

  *response = (unsigned char *)malloc(response_size);
  AES_GCM_128_decrypt(encrypted_response, response_size, NULL, 0, tag, aes_key,
                      iv, IV_LENGTH, *response);
  return response_size;
}

int encryptBulkReadRequest(uint32_t *rs, uint32_t req_counter,
                           uint32_t bulk_batch_size,
                           unsigned char *encrypted_request, unsigned char *tag,
                           uint32_t request_size, unsigned char *iv,
  ...
  *serialized_client_public_key =
      (unsigned char *)malloc(size_bin_x + size_bin_y);
  ...
  */
  ...
  // Returns a 32 byte secret
  secret_len = ECDH_compute_key(secret, secret_len,
                                EC_KEY_get0_public_key(ENCLAVE_PUBLIC_KEY),
                                ephemeral_key, NULL);
  ...
  */
  ...
  // Sample IV;
  unsigned char *serialized_request =
      (unsigned char *)malloc(encrypted_request_size);
  ...
  encrypted_request_size = AES_GCM_128_encrypt(
      serialized_request, request_size, NULL, 0,
      (unsigned char *)ecdh_shared_aes_key, (unsigned char *)ecdh_shared_iv,
...
void decryptBulkReadRequest(uint32_t bulk_batch_size,
                            unsigned char *encrypted_request,
                            unsigned char *tag, uint32_t request_size) {
  unsigned char *decrypted_request = (unsigned char *)malloc(request_size);
  int decrypted_request_size = AES_GCM_128_decrypt(
      encrypted_request, request_size, NULL, 0, tag,
      (unsigned char *)SHARED_AES_KEY, (unsigned char *)HARDCODED_IV, IV_LENGTH,
      decrypted_request);
  uint32_t request_id;
...
uint32_t computeCiphertextSize(uint32_t data_size) {
  ...
  uint32_t encrypted_request_size;
  encrypted_request_size =
      ((1 + ID_SIZE_IN_BYTES + data_size) / AES_GCM_BLOCK_SIZE_IN_BYTES);
...
uint32_t computeBulkRequestsCiphertextSize(uint32_t bulk_batch_size) {
  uint32_t encrypted_request_size;
  encrypted_request_size =
      ((ID_SIZE_IN_BYTES * bulk_batch_size) / AES_GCM_BLOCK_SIZE_IN_BYTES);
...
void decryptRequest(unsigned char *encrypted_request, unsigned char *tag,
                    uint32_t request_size) {
...
int extractResponse(unsigned char *encrypted_response, unsigned char *tag,
                    int response_size, unsigned char *data_out) {
  AES_GCM_128_decrypt(encrypted_response, response_size, NULL, 0, tag,
                      (unsigned char *)SHARED_AES_KEY,
                      (unsigned char *)HARDCODED_IV, IV_LENGTH, data_out);
  return response_size;
}
