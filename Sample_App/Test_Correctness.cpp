...
void initializeZeroTrace() {
  ...
  unsigned char bin_x[PRIME256V1_KEY_SIZE], bin_y[PRIME256V1_KEY_SIZE],
      signature_r[PRIME256V1_KEY_SIZE], signature_s[PRIME256V1_KEY_SIZE];
  ...
  unsigned char *serialized_public_key =
      (unsigned char *)malloc(PRIME256V1_KEY_SIZE * 2);
  memcpy(serialized_public_key, bin_x, PRIME256V1_KEY_SIZE);
  memcpy(serialized_public_key + PRIME256V1_KEY_SIZE, bin_y,
         PRIME256V1_KEY_SIZE);
  ...
  // New syntax
  ECDSA_SIG_set0(sig_enclave, BN_bin2bn(signature_r, PRIME256V1_KEY_SIZE, NULL),
                 BN_bin2bn(signature_s, PRIME256V1_KEY_SIZE, NULL));

  ret = ECDSA_do_verify((const unsigned char *)serialized_public_key,
                        PRIME256V1_KEY_SIZE * 2, sig_enclave,
                        enclave_verification_key);
  if (ret == 1) {
...
int run_experiment(exp_params params) {
  ...
  uint32_t zt_id = ZT_New(max_blocks, data_size, stash_size, oblivious_flag,
  ...
  printf("Starting ORAM writes to store unique data into each index before "
         "performing read requests.\n");
#endif

  // Write data blocks for indices i: 0 to MAX_BLOCKS-1, as encryption of index
  // padded with 0s to fill DATA_SIZE
  for (i = 0; i < max_blocks; i++) {
    ...
    encryptRequest(insert_seq[i], 'w', data_in, data_size, encrypted_request,
                   tag_in, encrypted_request_size);

    // Perform the ORAM write
    ZT_Access(zt_id, oram_type, encrypted_request, encrypted_response, tag_in,
              tag_out, encrypted_request_size, response_size, TAG_SIZE);
  }
  ...
  for (i = 0; i < request_length; i++) {
    ...
    encryptRequest(rs[i], 'r', data_in, data_size, encrypted_request, tag_in,
  ...
  if (mb_recursion_levels == 1) {
    ...
    double download_time_avg, download_time_std, posmap_time_avg,
        posmap_time_std, fetch_block_time_avg, fetch_block_time_std,
        eviction_time_avg, eviction_time_std, upload_time_avg, upload_time_std,
        total_time_avg, total_time_std;

    double posmap_time[mb_request_length],
        download_time_level[mb_request_length],
        fetch_block_time_level[mb_request_length],
        eviction_time_level[mb_request_length],
    ...
    download_time_avg =
        compute_avg((double *)download_time_level, mb_request_length);
    download_time_std =
        compute_stddev((double *)download_time_level, mb_request_length);
    ...
    fetch_block_time_avg =
        compute_avg((double *)fetch_block_time_level, mb_request_length);
    fetch_block_time_std =
        compute_stddev((double *)fetch_block_time_level, mb_request_length);
    ...
    eviction_time_avg =
        compute_avg((double *)eviction_time_level, mb_request_length);
    eviction_time_std =
        compute_stddev((double *)eviction_time_level, mb_request_length);
    ...
    upload_time_avg =
        compute_avg((double *)upload_time_level, mb_request_length);
    upload_time_std =
        compute_stddev((double *)upload_time_level, mb_request_length);
    ...
    fprintf(log_file_avg, "%f, %f, %f, %f, %f, %f\n", posmap_time_avg,
            download_time_avg, fetch_block_time_avg, eviction_time_avg,
            upload_time_avg, total_time_avg);
    fclose(log_file_avg);

    FILE *log_file_std = fopen(LOG_FILE_STD.c_str(), "w");
    fprintf(log_file_std, "%f, %f, %f, %f, %f, %f\n", posmap_time_std,
            download_time_std, fetch_block_time_std, eviction_time_std,
            upload_time_std, total_time_std);
    fclose(log_file_std);
  } else {
    // Position Map time is only in level 0, so no need to iterate over
    // recursion_levels;
    ...
    for (uint32_t j = 0; j < recursion_levels; j++) {
      ...
      printf("Download_time_avg[%d] = %lf, Download_time_std[%d] = %lf\n", j,
             download_time_avg[j], j, download_time_std[j]);
    }
    ...
    for (uint32_t j = 0; j < recursion_levels; j++) {
      ...
      fetch_block_time_avg[j] =
          compute_avg((double *)fetch_block_time_level, mb_request_length);
      fetch_block_time_std[j] =
          compute_stddev((double *)fetch_block_time_level, mb_request_length);
    ...
    for (uint32_t j = 0; j < recursion_levels; j++) {
      ...
      eviction_time_std[j] =
          compute_stddev((double *)eviction_time_level, mb_request_length);
      printf("Eviction_logic_time_avg[%d] = %lf, Eviction_logic_time_std[%d] = "
             "%lf\n",
    ...
    for (uint32_t j = 0; j < recursion_levels; j++) {
      ...
      // upload_time_avg[j], j, upload_time_std[j]);
    }
  }
  ...
  // fetch the pointers to these structures and then display the results as
  // required
  ...
  printf("Per query time = %f ms\n",
         (1000 * ((double)tclock / ((double)request_length)) /
...
void getParams(int argc, char *argv[]) {
  if (argc != NUM_EXPECTED_PARAMS) {
    printf("Command line parameters error, received: %d, expected :%d\n", argc,
           NUM_EXPECTED_PARAMS);
    printf(" <N> <No_of_requests> <Stash_size> <Data_block_size> "
           "<Recursion_block_size> <\"path(0)\"/\"circuit(1)\"> <Z> <LogFolder "
           "(Must Exist)>\n\n");
    exit(0);
  }
  ...
  if (ORAM_TYPE == 0)
    ...
    ot = "CO";
  LOG_FILE = LOG_FOLDER + '/' + ot + "_" + std::to_string(MAX_BLOCKS) + "_" +
             std::to_string(DATA_SIZE) + "_" + std::to_string(STASH_SIZE) +
             "_" + std::to_string(Z) + "_" + std::to_string(REQUEST_LENGTH);
  printf("LOG_FILE = %s", LOG_FILE.c_str());
  // std::string qfile_name =
  // "ZT_"+std::to_string(MAX_BLOCKS)+"_"+std::to_string(DATA_SIZE); iquery_file
  // = fopen(qfile_name.c_str(),"w");
}
#endif
...
int main(int argc, char *argv[]) {
  ...
  // DATA_SIZE, MAX_BLOCKS, REQ_LENGTH, STASH_SIZE, OBLIVIOUS_FLAG,
  // RECURSION_DATA_SIZE, ORAM_TYPE, Z
