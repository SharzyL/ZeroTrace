...
void getParams(int argc, char *argv[]) {
  if (argc < min_expected_no_of_parameters) {
    printf("Command line parameters error, received %d,  expected %d:\n", argc,
           min_expected_no_of_parameters);
    printf(" <N> <No_of_requests> <key_size> <value_size> <0/1 = Store "
           "In-PRM/Outside-PRM> <0/1 = Access-Oblivious/Full-Oblivious> <ORAM "
           "Type(\"circuit\" or \"path\")> <stash_size> <double_oblivious> "
           "<recursion_data_size> <Z=4> <Logfile>\n");
  }
...
void client_HSORAM_Insert(uint32_t lsoram_iid, uint32_t oram_iid,
  ...
  */
  ...
  ZT_HSORAM_insert(lsoram_iid, oram_iid, oram_type, oram_index++,
...
void client_HSORAM_Fetch(uint32_t lsoram_iid, uint32_t oram_iid,
                         unsigned char *key, uint32_t key_size,
                         unsigned char *encrypted_value, uint32_t value_size,
  ...
  */
  ...
  decryptLSORAMResponse(encrypted_value, value_size, tag_out, ecdh_aes_key, iv,
                        &response);
...
int main(int argc, char *argv[]) {
  ...
  zt_lsoram_id = ZT_New_LSORAM(num_blocks, key_size, INDEX_SIZE, store_mode,
                               oblivious_mode, 1);
  ...
  for (int i = 0; i < num_blocks; i++) {
    ...
    client_HSORAM_Insert(zt_lsoram_id, zt_oram_id, key, key_size, value,
                         value_size);
  ...
  // TODO: Send requests for inserted keys, check that value returned matches
  // the one in map
  ...
  for (int i = 0; i < requestlength; i++) {
    ...
    fetches_start = clock();
    client_HSORAM_Fetch(zt_lsoram_id, zt_oram_id, key, key_size,
