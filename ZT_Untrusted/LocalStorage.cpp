...
// Utilization Parameter is the number of blocks of a bucket that is filled at
// start state. ( 4 = MAX_OCCUPANCY )
// #define NO_CACHING 1
...
int8_t LocalStorage::restoreState(uint32_t *posmap, uint32_t posmap_size,
                                  uint32_t *stash, uint32_t *stash_size,
                                  unsigned char *merkle_root,
                                  uint32_t hash_and_key_size) {
...
void LocalStorage::savePosmapMerkleRoot(unsigned char *posmap,
                                        uint32_t posmap_size,
                                        unsigned char *merkle_root_and_aes_key,
                                        uint32_t hash_and_key_size) {
  std::string fpp = directoryFP + "posmap";
  std::string fpr = directoryFP + "merkleroot";
  try {
...
void LocalStorage::saveStashLevel(unsigned char *stash, uint32_t stash_size,
                                  uint8_t level) {
...
void LocalStorage::saveState(unsigned char *posmap, uint32_t posmap_size,
                             unsigned char *stash, uint32_t stash_size,
                             unsigned char *merkle_root_and_aes_key,
                             uint32_t hash_and_key_size) {
...
void LocalStorage::setParams(uint32_t max_blocks, uint32_t set_D,
                             uint32_t set_Z, uint32_t stash_size,
                             uint32_t data_size_p, bool inmem_p,
                             uint32_t recursion_block_size_p,
                             uint8_t recursion_levels_p) {
  ...
  temp = std::to_string(max_blocks) + "_" + std::to_string(data_block_size) +
         "_" + std::to_string(stash_size);
...
#ifdef DEBUG_LS
  printf("\nIN LS : recursion_levels = %d, data_size = %d, "
         "recursion_block_size = %d\n\n",
         recursion_levels, data_block_size, recursion_block_size);
  printf("DataTree_size = %ld or %f GB, HashTree_size = %ld or %f GB\n",
         datatree_size, float(datatree_size) / (float(1024 * 1024 * 1024)),
         hashtree_size, float(hashtree_size) / (float(1024 * 1024 * 1024)));
#endif
  ...
  if (inmem == false) {
    ...
    if (recursion_levels == 0) {
      ...
      blocks_in_level =
          (uint64_t *)malloc((recursion_levels + 1) * sizeof(uint64_t *));
      ...
      inmem_tree_l = (unsigned char **)malloc((recursion_levels) *
                                              sizeof(unsigned char *));
      inmem_hash_l = (unsigned char **)malloc((recursion_levels) *
                                              sizeof(unsigned char *));

      for (uint32_t i = 1; i <= recursion_levels; i++) {
        std::string file_name_this = file_name + "p" + std::to_string(i);
        std::string file_name_this_i =
            file_name + "p" + std::to_string(i) + "_i";
        ...
        uint64_t file_size, remainder, mem_tree_size, next_layer_size,
            final_file_size = 0;
        uint64_t hashtree_size_this =
            (uint64_t)(ptreeSize) * (uint64_t)HASH_LENGTH;

        inmem_hash_l[i] = (unsigned char *)malloc(hashtree_size_this);
        printf("HASHTREE_SIZE for level %d = %f MB\n", i,
               float(hashtree_size_this) / float(1024 * 1024));
        ...
        if (i == recursion_levels)
          ...
          file_size =
              (uint64_t)ptreeSize * (uint64_t)(Z * recursion_block_size);
        ...
        if (i == recursion_levels) {
          ...
          // If tree doesn't fit in memory more of the lower levels are moved to
          // Disk
          while (mem_tree_size > remainder) {
            next_layer_size = ((uint64_t)(pow(2, pD - (levels_on_disk))) *
                               (uint64_t)Z * data_size_p);
      ...
      for (uint32_t i = 1; i <= recursion_levels; i++) {
        ...
        printf("Level = %d, MaxBlocks = %ld, File_size = %ld or %f GB\n", i,
               blocks_in_level[i], file_size,
               float(file_size) / float(1024 * 1024 * 1024));
        file.write("X", 1);
        file.close();

        uint64_t hashtree_size_this =
            (uint64_t)(pow(2, pD + 1) - 1) * (uint64_t)HASH_LENGTH;
    ...
    if (recursion_levels == 1) {
      ...
      uint32_t x =
          (recursion_block_size - ADDITIONAL_METADATA_SIZE) / sizeof(uint32_t);
#ifdef DEBUG_LS
      printf("X = %d\n", x);
#endif
      ...
      blocks_in_level = (uint64_t*) malloc((recursion_levels) *
      sizeof(uint64_t*)); blocks_in_level[recursion_levels-1] = pmap0_blocks;

      while(level > 0) {
        blocks_in_level[level-1] =
      ceil((double)blocks_in_level[level]/(double)x); level--;
      }
      */
      ...
      for (uint32_t i = 0; i < recursion_levels; i++) {
        uint64_t level_size;
        // leaf_nodes in level = ceil(log_2(ceil(blocks_in_level[level] /
        // util_divisor)) where util_divisor = Z Total nodes in Tree = 2
        // *leaf_nodes -1
        uint32_t pD_temp = ceil((double)blocks_in_level[i] / (double)Z);
        uint32_t pD = (uint32_t)ceil(log((double)pD_temp) / log((double)2));
        ...
        if (i == recursion_levels - 1)
          level_size =
              tree_size *
              ((uint64_t)(Z * (data_size_p + ADDITIONAL_METADATA_SIZE)));
        else
          level_size = tree_size * ((uint64_t)(Z * (recursion_block_size +
                                                    ADDITIONAL_METADATA_SIZE)));

        uint64_t hashtree_size =
            (uint64_t)(tree_size * (uint64_t)(HASH_LENGTH));
...
#ifdef SET_PARAMETERS_DEBUG
        printf(
            "LS:Level : %d, Blocks : %ld, TreeSize = %ld, hashtree_size= %ld\n",
...
void LocalStorage::fetchHash(uint32_t bucket_id, unsigned char *hash,
                             uint32_t hash_size, uint8_t recursion_level) {
  ...
  if (inmem == false) {
#ifdef CACHE_UPPER
    memcpy(hash,
           inmem_hash_l[recursion_level] +
               ((uint64_t)(bucket_id - 1) * (uint64_t)HASH_LENGTH),
           HASH_LENGTH);
#else
    ...
    memcpy(hash,
           inmem_hash_l[recursion_level] + ((bucket_id - 1) * HASH_LENGTH),
           HASH_LENGTH);
  }
}

uint8_t LocalStorage::uploadBucket(uint32_t bucket_id, unsigned char *bucket,
                                   uint32_t bucket_size, unsigned char *hash,
                                   uint32_t hash_size,
                                   uint8_t recursion_level) {
#ifdef DEBUG_LS
  printf("LS:uploadBucket : level = %d, bucket_id = %d of buckets_in_level[%d] "
         "= %ld\n",
         recursion_level, bucket_id, recursion_level,
         buckets_in_level[recursion_level]);
#endif
  ...
  if (inmem == false) {
    try {
      ...
      printf("Level : %d, %s, Pos : %d\n", recursion_level,
             file_name_this.c_str(), (bucket_id - 1) * Z * bucket_size);
#endif
      ...
      */
      ...
      posix_fadvise(filedesc, (bucket_id - 1) * hash_size, hash_size,
                    POSIX_FADV_DONTNEED);
      syncfs(filedesc);
      close(filedesc);
      ...
      if (recursion_level == recursion_levels) {
        if (bucket_id <= objectkeylimit) {
          ...
          memcpy(inmem_hash_l[recursion_level] +
                     ((uint64_t)HASH_LENGTH * (uint64_t)(bucket_id - 1)),
                 hash, hash_size);
        } else {
          ...
          memcpy(inmem_hash_l[recursion_level] +
                     ((uint64_t)HASH_LENGTH * (uint64_t)(bucket_id - 1)),
                 hash, hash_size);
          pos = (uint64_t)(bucket_id - 1 - objectkeylimit) *
                (uint64_t)(Z * bucket_size);
          // std::cout<<"Pos = "<<pos<<"\n";
          std::ofstream file(file_name_this.c_str(),
        ...
        memcpy(inmem_tree_l[recursion_level] +
                   ((Z * bucket_size) * (bucket_id - 1)),
               data, (Z * bucket_size));
        memcpy(inmem_hash_l[recursion_level] + (HASH_LENGTH * (bucket_id - 1)),
               hash, hash_size);
      }
#else
      std::ofstream file(file_name_this.c_str(),
...
void LocalStorage::showPath_reverse(unsigned char *decrypted_path, uint8_t Z,
...
uint8_t LocalStorage::uploadPath(uint32_t leaf_label, unsigned char *path,
                                 unsigned char *path_hash, uint8_t level,
                                 uint32_t D_level) {
  std::string file_name_this, file_name_this_i;
  ...
  if (inmem == false) {
      ...
      try {
        ...
        if (level == recursion_levels) {
          if (temp > objectkeylimit) {
            ...
            memcpy(inmem_tree_l[level] +
                       ((uint64_t)(Z * size_for_level) * (uint64_t)(temp - 1)),
                   path_iter, (Z * size_for_level));
            path_iter += (Z * size_for_level);
          }

          // Common integrity tree part
          memcpy(inmem_hash_l[level] + (uint64_t)(HASH_LENGTH * (temp - 1)),
                 path_hash_iter, HASH_LENGTH);
          path_hash_iter += (HASH_LENGTH);
          ...
          memcpy(inmem_tree_l[level] + ((Z * size_for_level) * (temp - 1)),
                 path_iter, (Z * size_for_level));
          memcpy(inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)),
                 path_hash_iter, HASH_LENGTH);
          path_iter += (Z * size_for_level);
          path_hash_iter += HASH_LENGTH;
        }
    ...
    */
    ...
    for (uint8_t i = 0; i < D_level; i++) {
      ...
      for (uint8_t q = 0; q < Z; q++) {
        printf("(%d,%d),",
               *((uint32_t *)(inmem_tree_l[level] +
                              ((Z * size_for_level) * (temp - 1) +
                               q * size_for_level + 16))),
               *((uint32_t *)(inmem_tree_l[level] +
                              ((Z * size_for_level) * (temp - 1) +
                               q * size_for_level + 20))));
      ...
      memcpy(inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)), path_hash_iter,
             HASH_LENGTH);
      /*
...
// unsigned char* downloadBucket(uint32_t bucket_id, unsigned char* data,
// unsigned char *hash, uint32_t hash_size, uint8_t level, uint32_t D_lev);

uint8_t LocalStorage::downloadBucket(uint32_t bucket_id, unsigned char *bucket,
                                     uint32_t bucket_size, unsigned char *hash,
                                     uint32_t hash_size, uint8_t level) {
  ...
  memcpy(bucket, inmem_tree_l[level] + ((Z * size_for_level) * (bucket_id - 1)),
         (Z * size_for_level));
  memcpy(hash, inmem_hash_l[level] + (HASH_LENGTH * (bucket_id - 1)),
         HASH_LENGTH);
...
For each node on path, returned path_hash contains <L-hash, R-Hash> pairs with
the exception of a single hash for root node

*/

unsigned char *LocalStorage::downloadPath(uint32_t leaf_label,
                                          unsigned char *path,
                                          unsigned char *path_hash,
                                          uint32_t path_hash_size,
                                          uint8_t level, uint32_t D_lev) {
  uint64_t pos;
  std::string file_name_this, file_name_this_i;
  uint32_t size_for_level;
    ...
    for (uint8_t i = 0; i < D_lev + 1; i++) {
      ...
      try {
        ...
        if (temp == 1) {
          ...
          pread(filedesc, path_hash_iter, HASH_LENGTH,
                (temp - 1) * HASH_LENGTH);
          path_hash_iter += (HASH_LENGTH);
          close(filedesc);
        } else {
          ...
          pread(filedesc, path_hash_iter, HASH_LENGTH,
                (temp_sib - 1) * HASH_LENGTH);
          path_hash_iter += (HASH_LENGTH);
          posix_fadvise(filedesc, (temp - 1) * HASH_LENGTH, HASH_LENGTH * 2,
                        POSIX_FADV_DONTNEED);
          close(filedesc);
        }
        ...
        if (level == recursion_levels) {
          if (temp > objectkeylimit) {
            ...
            pos = (uint64_t)(temp - 1) * (uint64_t)(size_for_level * Z);
            memcpy(path_iter, (inmem_tree_l[level]) + pos,
                   (Z * size_for_level));
            path_iter += (Z * size_for_level);
          }
          ...
          // Common integrity tree part
          if (temp == 1) {
            memcpy(path_hash_iter,
                   inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)),
                   HASH_LENGTH);
            path_hash_iter += (HASH_LENGTH);
          } else {
            if (temp % 2 == 0)
            ...
            path_hash_iter += (HASH_LENGTH);
            memcpy(path_hash_iter,
                   inmem_hash_l[level] + (HASH_LENGTH * (temp_sib - 1)),
                   HASH_LENGTH);
            path_hash_iter += (HASH_LENGTH);
          }
        } else {
...
#endif
          memcpy(path_iter,
                 inmem_tree_l[level] + ((Z * size_for_level) * (temp - 1)),
                 (Z * size_for_level));
          memcpy(path_hash_iter,
                 inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)), HASH_LENGTH);
          path_iter += (Z * size_for_level);
          path_hash_iter += HASH_LENGTH;
        }
#else
        ...
        // file_name_this.c_str(),pos, temp-1); std::string fp = directoryFP +
        // std::to_string(temp);
      ...
      if (i != D_lev - 1) {
        if (temp % 2 == 0) {
          memcpy(path_hash_iter,
                 inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)), HASH_LENGTH);
          path_hash_iter += HASH_LENGTH;
          memcpy(path_hash_iter, inmem_hash_l[level] + (HASH_LENGTH * (temp)),
                 HASH_LENGTH);
          path_hash_iter += HASH_LENGTH;
          ...
          for (uint8_t l = 0; l < HASH_LENGTH; l++)
            printf(
                "%c",
                (*(inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)) + l) % 26) +
                    'A');
          printf("\nLS : Level = %d, Bucket no = %d, Hash = ", level, temp + 1);
          for (uint8_t l = 0; l < HASH_LENGTH; l++)
            printf("%c",
                   (*(inmem_hash_l[level] + (HASH_LENGTH * (temp)) + l) % 26) +
                       'A');
          printf("\n");
#endif
        } else {
          memcpy(path_hash_iter,
                 inmem_hash_l[level] + (HASH_LENGTH * (temp - 2)), HASH_LENGTH);
          path_hash_iter += HASH_LENGTH;
          memcpy(path_hash_iter,
                 inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)), HASH_LENGTH);
          path_hash_iter += HASH_LENGTH;
          ...
          for (uint8_t l = 0; l < HASH_LENGTH; l++)
            printf(
                "%c",
                (*(inmem_hash_l[level] + (HASH_LENGTH * (temp - 2)) + l) % 26) +
                    'A');
        ...
        memcpy(path_hash_iter, inmem_hash_l[level] + (HASH_LENGTH * (temp - 1)),
               HASH_LENGTH);
        path_hash_iter += (HASH_LENGTH);
#ifdef LS_DEBUG_INTEGRITY
        printf("LS : Level = %d, Bucket no = %d, Hash = ", level, temp);
