...
Usage :
printf("./app <N> <No_of_requests> <Stash_size> <Data_block_size>
<"resume"/"new"> <"memory"/"hdd"> <"0"/"1" = Non-oblivious/Oblivious>
<Recursion_block_size>"); Note : parameters surrounded by quotes should entered
in as is without the quotes.
...
int initialize_enclave(void) {
  ...
  if (fp == NULL && (fp = fopen(token_path, "wb")) == NULL) {
    fprintf(stderr,
            "ZT_LSORAM:Warning: Failed to create/open the launch token file "
            "\"%s\".\n",
            token_path);
  }
  ...
  if (fp != NULL) {
    ...
    if (read_num != 0 && read_num != sizeof(sgx_launch_token_t)) {
      ...
      fprintf(stderr,
              "ZT_LSORAM:Warning: Invalid launch token read from \"%s\".\n",
              token_path);
    }
  }
  /* Step 2: call sgx_create_enclave to initialize an enclave instance */
...
void *HandleRequest(void *arg) {
  ...
  while (1) {
    ...
    ls.downloadPath(*id, data->resp->path, data->resp->path_hash,
                    path_hash_size, *level, *d_lev);
    // printf("APP : Downloaded Path\n");
    *(data->req->block) = true;

    while (*(data->req->block)) {
    }
    ls.uploadPath(*id, data->resp->new_path, data->resp->new_path_hash, *level,
                  *d_lev);
...
uint32_t ZT_New_LSORAM(uint32_t num_blocks, uint32_t key_size,
                       uint32_t value_size, uint8_t mode,
                       uint8_t oblivious_type, uint8_t populate_flag) {
  sgx_status_t sgx_return;
  uint32_t instance_id;
  sgx_return =
      createNewLSORAMInstance(global_eid, &instance_id, key_size, value_size,
                              num_blocks, mode, oblivious_type, populate_flag);

  return instance_id;
}

int8_t ZT_LSORAM_insert(uint32_t instance_id, unsigned char *encrypted_request,
  ...
  int8_t ret;
  sgx_return =
      LSORAMInsert(global_eid, &ret, instance_id, encrypted_request,
                   request_size, tag_in, tag_size, client_pubkey,
...
int8_t ZT_LSORAM_oprm_insert_pt(uint32_t instance_id, unsigned char *key_l,
                                uint32_t key_size, unsigned char *value_l,
                                uint32_t value_size) {
...
int8_t ZT_LSORAM_iprm_insert_pt(uint32_t instance_id, unsigned char *key_l,
  ...
  int8_t ret;
  sgx_status_t sgx_return;
  sgx_return = LSORAMInsert_pt(global_eid, &ret, instance_id, key_l, key_size,
                               value_l, value_size);
...
int8_t ZT_LSORAM_fetch(uint32_t instance_id, unsigned char *encrypted_request,
  ...
  int8_t ret;
  sgx_return =
      LSORAMFetch(global_eid, &ret, instance_id, encrypted_request,
                  request_size, encrypted_response, response_size, tag_in,
                  tag_out, tag_size, client_pubkey,
                  pubkey_size_x + pubkey_size_y, pubkey_size_x, pubkey_size_y);
  return ret;
}

int8_t ZT_HSORAM_insert(uint32_t lsoram_iid, uint32_t oram_iid,
                        uint8_t oram_type, uint64_t oram_index,
  ...
  sgx_status_t sgx_return;
  int8_t ret;
  sgx_return = HSORAMInsert(
      global_eid, &ret, lsoram_iid, oram_iid, oram_type, oram_index,
      encrypted_request, request_size, tag_in, tag_size, client_pubkey,
      pubkey_size_x + pubkey_size_y, pubkey_size_x, pubkey_size_y);
  return ret;
}

int8_t ZT_HSORAM_fetch(uint32_t lsoram_iid, uint32_t oram_iid,
                       uint8_t oram_type, unsigned char *encrypted_request,
                       uint32_t request_size, unsigned char *encrypted_response,
                       uint32_t response_size, unsigned char *tag_in,
                       unsigned char *tag_out, uint32_t tag_size,
                       unsigned char *client_pubkey, uint32_t pubkey_size_x,
  ...
  int8_t ret;
  sgx_return =
      HSORAMFetch(global_eid, &ret, lsoram_iid, oram_iid, oram_type,
                  encrypted_request, request_size, encrypted_response,
                  response_size, tag_in, tag_out, tag_size, client_pubkey,
                  pubkey_size_x + pubkey_size_y, pubkey_size_x, pubkey_size_y);
  return ret;
}
...
void *createLSORAM_OCALL(uint32_t id, uint32_t key_size, uint32_t value_size,
                         uint32_t num_blocks_p, uint8_t oblv_mode) {
...
uint8_t uploadPath_OCALL(uint32_t instance_id, uint8_t oram_type,
                         unsigned char *path_array, uint32_t path_size,
                         uint32_t leaf_label, unsigned char *path_hash,
                         uint32_t path_hash_size, uint8_t level,
                         uint32_t D_level) {
  LocalStorage *ls;
  if (oram_type == 0) {
...
uint8_t uploadBucket_OCALL(uint32_t instance_id, uint8_t oram_type,
                           unsigned char *serialized_bucket,
                           uint32_t bucket_size, uint32_t label,
                           unsigned char *hash, uint32_t hashsize,
                           uint32_t size_for_level, uint8_t recursion_level) {
  LocalStorage *ls;
  if (oram_type == 0) {
    auto search = ls_PORAM.find(instance_id);
    if (search != ls_PORAM.end()) {
      ls = search->second;
      ls->uploadBucket(label, serialized_bucket, size_for_level, hash, hashsize,
                       recursion_level);
    } else {
...
uint8_t downloadPath_OCALL(uint32_t instance_id, uint8_t oram_type,
                           unsigned char *path_array, uint32_t path_size,
                           uint32_t leaf_label, unsigned char *path_hash,
                           uint32_t path_hash_size, uint8_t level,
                           uint32_t D_level) {
  ...
  if (oram_type == 0) {
    ...
    ls->downloadPath(leaf_label, path_array, path_hash, path_hash_size, level,
                     D_level);
  } else if (oram_type == 1) {
    auto search = ls_CORAM.find(instance_id);
    if (search != ls_CORAM.end()) {
...
uint8_t downloadBucket_OCALL(uint32_t instance_id, uint8_t oram_type,
                             unsigned char *serialized_bucket,
                             uint32_t bucket_size, uint32_t label,
                             unsigned char *hash, uint32_t hashsize,
                             uint32_t size_for_level, uint8_t recursion_level) {
  ...
  if (oram_type == 0) {
    ...
    ls->downloadBucket(label, serialized_bucket, size_for_level, hash, hashsize,
                       recursion_level);
  } else if (oram_type == 1) {
    auto search = ls_CORAM.find(instance_id);
    if (search != ls_CORAM.end()) {
...
void build_fetchChildHash(uint32_t instance_id, uint8_t oram_type,
                          uint32_t left, uint32_t right, unsigned char *lchild,
                          unsigned char *rchild, uint32_t hash_size,
...
uint8_t computeRecursionLevels(uint32_t max_blocks,
                               uint32_t recursion_data_size,
                               uint64_t onchip_posmap_memory_limit) {
...
uint8_t getRecursionLevels() { return (DET_MB_PARAMS.recursion_levels); }
...
void ZT_Close() { sgx_destroy_enclave(global_eid); }
...
uint32_t ZT_New(uint32_t max_blocks, uint32_t data_size, uint32_t stash_size,
  ...
  printf("App.cpp: Parmas for LS : \n \(%d, %d, %d, %d, %d, %d, %d, %d)\n",
         max_blocks, D, pZ, stash_size, data_size + ADDITIONAL_METADATA_SIZE,
         inmem_flag, recursion_data_size + ADDITIONAL_METADATA_SIZE,
         recursion_levels);

  // LocalStorage Module, just works with recursion_levels 0 to recursion_levels
  // And functions without treating recursive and non-recursive backends
  // differently Hence recursion_levels passed = recursion_levels,

  ls_oram->setParams(max_blocks, D, pZ, stash_size,
                     data_size + ADDITIONAL_METADATA_SIZE, inmem_flag,
                     recursion_data_size + ADDITIONAL_METADATA_SIZE,
                     recursion_levels);
  ...
  *(req_struct.id) = 7;
  ...
  uint8_t ret;
  sgx_return = createNewORAMInstance(
      global_eid, &ret, instance_id, max_blocks, data_size, stash_size,
      oblivious_flag, recursion_data_size, recursion_levels, oram_type, pZ);
...
void ZT_Access(uint32_t instance_id, uint8_t oram_type,
               ...
               unsigned char *tag_out, uint32_t request_size,
  ...
  accessInterface(global_eid, instance_id, oram_type, encrypted_request,
                  encrypted_response, tag_in, tag_out, request_size,
                  response_size, tag_size);
...
void ZT_Bulk_Read(uint32_t instance_id, uint8_t oram_type,
                  uint32_t no_of_requests, unsigned char *encrypted_request,
                  unsigned char *encrypted_response, unsigned char *tag_in,
                  unsigned char *tag_out, uint32_t request_size,
                  uint32_t response_size, uint32_t tag_size) {
  accessBulkReadInterface(global_eid, instance_id, oram_type, no_of_requests,
                          encrypted_request, encrypted_response, tag_in,
                          tag_out, request_size, response_size, tag_size);
}
...
if(resume_experiment) {
    ...
    ls.restoreState(posmap, max_blocks, stash, &current_stashSize, merkle,
hash_size+aes_key_size);
    //sgx_return = restore_enclave_state(global_eid, &rt32, max_blocks,
dataSize_p, posmap, posmap_size, stash, current_stashSize * 8, merkle,
hash_size+aes_key_size);
    //printf("Restore done\n");
    free(posmap);
    free(stash);
